...
try {
    ...
    if ($_GET['action'] === 'getallcourse') {
        echo json_encode(getCourseLists(), JSON_UNESCAPED_UNICODE);
    } else {
...
{
    global $DB;
    $assessment = $DB->query('SELECT * FROM asm_assetmentform');
    $emptyArray = [];
    foreach ($assessment as $key => $value) {
        array_push($emptyArray, [
            'assessment_id' => intval($value['id']),
            'course_id' => intval($value['course_id']),
            'course_shortname' => getCourseShortName($value['course_id']),
            'teacher_id' => intval($value['teacher_id']),
            'teacher_name' => getUserName($value['teacher_id']),
        ]);
    }
    return $emptyArray;
}
