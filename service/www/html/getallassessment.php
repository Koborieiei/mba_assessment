...
try {
    $DB = new DB();
    $LMS = new Lms();
    $assessment_id = (!isset($_GET['asm_id']) ? rand(6, 7) : $_GET['asm_id']);
    $DB->bind('assessmentId', $assessment_id);
    ...
    FROM (SELECT a.transection_id,b.assetmentform_id,a.score,c.question_text,c.question_type
        ...
        WHERE assetmentform_id = :assessmentId
    ...
    foreach ($queryData as $value) {
        ...
        // Get average score for each one
        $avg = array_reduce($test, function ($acc, $item) {
            return $acc + $item['total'] * $item['score'];
        });

          // Get all number of answers
        $finalTotalScore = array_reduce($tempArray['questions'][$indexOfArray]['totalscore'], function ($acc, $item) {
            return $acc + $item['total'];
        });

        $tempArray['questions'][$indexOfArray]['totalavg'] = round($avg / $finalTotalScore, 2);

    }

    // Get summary of Average score
    $finalAverageSummary = array_reduce($tempArray['questions'], function ($acc, $item) {
        return $acc + $item['totalavg'];
    });

    $tempArray['comments'] = getAllAsessmentComments($assessment_id);
    $countQuestion = count($tempArray['questions']);
    $finaltotalscore = $countQuestion === 0 ? 0 : $finalAverageSummary / $countQuestion;
    $tempArray['finaltotalsummary'] = round($finaltotalscore, 2);
